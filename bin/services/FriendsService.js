...
module.exports = {
    ...
    acceptFriendRequest: function(user, friend) {
        return this.friendRequestsPending(user).updateOne({user_id: friend._id},{$set:{accepted: true}})
    },
   ...
   hasSentRequest: function(user, friend) {
        return this.friendRequests(user).find({friend_id: friend._id}).countDocuments() 
     },
    ...
    hasFriendRequest: function(user, friend) {
        return this.friendRequestsPending(user).find({user_id: friend._id}).countDocuments() 
     },
    
     //check if you are friends with specific user (!!!check array length AFTER await!!!)
    isFriendWith : function(user, friend) {
      
        return this.friends(user).match(
           {friend_id: friend._id}
        ).count('count')  
    },
